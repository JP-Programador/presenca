    ...
    try {
      ...
      for (let i = 0; i < linhas.length; i++) {
        ...
        try {
          ...
          let mensagemCep: string | undefined;
          if (cep) {
            if (criado.latitude == null) {
              mensagemCep = "CEP não localizado em nenhum mapa — corrija em /colaboradores pro alerta de check-in funcionar.";
            } else if (criado.localizacao_precisao === "cidade") {
              mensagemCep = "CEP localizado só pelo centro da cidade (impreciso) — ajuste o pino em /colaboradores.";
            } else if (criado.localizacao_precisao === "bairro") {
              mensagemCep = "CEP localizado só por aproximação de bairro.";
            }
          }
          resultado.push({ linha: numeroLinha, nome, ok: true, mensagem: mensagemCep });
    ...
    <div className="flex flex-col gap-3">
      ...
      {avisosCep.length > 0 && (
        <div className="rounded-md border border-warning/30 bg-[#FFF3DB] px-4 py-3 text-sm text-[#8A6200]">
          <p>Atenção ao CEP em {avisosCep.length} linha(s) — importado mesmo assim:</p>
          <ul className="mt-2 list-disc pl-4 text-xs">
            {avisosCep.map((a) => (
              <li key={a.linha}>
                Linha {a.linha} ({a.nome}): {a.mensagem}
              </li>
            ))}
          </ul>
        </div>
